...
class SalesPartyBalance extends Component {
    constructor(props) {
        super(props);
        this.state = {
            current_month:{},
    ...
    getPartyInfo=()=>{
        ...
        this.props.distributer(obj,"getSalesPartyBalance").then(response => {
    ...
    calculateAggregate=(list)=>{
            ...
            if(!this.state.rejected_index.includes(ind)){
                if(row['entry_type']==="sales"){
                    ...
                    if(row['return']==="1"){
                        ...
                        total_tax=total_tax-parseFloat(row['tax'])
        ...
        let obj={
            ...
            effective:(total_amount+total_tax-credit_amount+debit_amount)
        }
            ...
            <div className=" main_div flex d-flex justify-content-center p-5">
                    ...
                    <Card className="col-lg-10">
                        <CardHeader className="bg-primary h5 text-light m-2">
                            ...
                            <PDFComponent data={currentList} date={current_month} partyInfo={salesPartyInfo} aggregate={current_aggregate} effective_balance={effective_balance} type="sales"></PDFComponent>
                        </CardHeader>
                                 ...
                                 <div className="col-lg-12">
                                    {
                                        ...
                                        item['entry_type']==="sales"?
                                            ...
                                            <div className="col-lg-1">{item['billno']}</div>
                                            {/* <div className="col-lg-3">{item['party']}</div> */}
                                            {/* <div className="col-lg-1">{item['bale']}</div> */}
                                            <div className="col-lg-1">{parseInt(item['weight']).toLocaleString('en-IN')}</div>
                                            <div className="col-lg-1">{item['rate']}</div>
                                            <div className="col-lg-2">{parseInt(item['amount']).toLocaleString('en-IN')}</div>
                                            <div className="col-lg-1">{parseInt(item['tax']).toLocaleString('en-IN')}</div>
                                            <div className="col-lg-2 font-weight-bold">{parseInt(item['total']).toLocaleString('en-IN')}</div>
                                        ...
                                        :
                                            ...
                                            <div className="col-lg-2">{item['date']}</div>   
                                            <div className="col-lg-1 font-weight-bold">{item['mode']}</div>
                                            {/* <div className="col-lg-3">{item['party']}</div> */}
                                            ...
                                            <div className="col-lg-2 font-weight-bold">{parseInt(item['amount']).toLocaleString('en-IN')}</div>
                                            <div className="col-lg-1">
                                                    <Input type="checkbox" className="p-1" onClick={e=>this.handle_rejection(ind)}></Input>
...
export default withRouter(connect(mapStateToProps, mapDispatchToProps)(SalesPartyBalance));